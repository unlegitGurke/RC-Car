...
\section{Motor und Motorcontroller}
	\subsection{Wahl des Motors}
		Wie schon in \ref{Motoren} Elektrische Antriebsmaschinen beschrieben, gibt es eine Vielzahl an möglichen Maschinen zum Antrieb eines solchen Fahrzeugs. Da die Motordrehzahl hier genau geregelt werden muss eignet sich die Synchronmaschine am besten. Verwendet wird eine BLDC-Syncrhonmaschine, da diese kostengünstiger wie eine PMSM ist, trotzdem aber eine ähnliche Charakteristik bietet. Um eine etwas bessere Kontrolle bei langsamen Drehzahlen zu erreichen werden außerdem Hall-Effekt-Sensoren am Motor angebracht. Der Hersteller TP-Power bietet zu einem günstigen Preis die Möglichkeit, Motoren auf Wunsch anzupassen und zu fertigen, gewählt wurder Motor TP5860 mit Hall-Effekt-Sensor er ist in folgenden Varianten verfügbar:
		...
		Die Motorkonfigurationen 8D und 4Y bilden ein gutes Gleichgewicht zwischen benötiger Spannung und benötigtem Strom, auch ist die Spannung perfekt durch eine 16S-LiPo Konfiguration erreichbar. Gewählt wurde im Endeffekt Konfiguration 8D
		...
		\subsection{Aufsetzen des Motorcontrollers}
			...
			\begin{enumerate}
				...
				\item Unter dem Reiter \textbf{Temperature} kann der Temperatursensor des Motors konfiguriert werden. Der Motor hat einen 10kOhm NTC verbaut, dieser wird ausgewählt, der Rest der Einstellungen können so bleiben, wie sie sind.
